...
To chown files, delete old output, and "copy" (via mount) output to tomcat, configure options in properties.yml, then run the following:
From terminal
       $ rake model_manager:copy_output
       
These commands require sudo and will prompt for pw. Of course these same tasks can also be run from the command line with no need for ruby. 
